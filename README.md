...
macOS에서 화면 기록을 허용한 뒤에는 Ouento를 완전히 종료하고 다시 여세요. 앱은 시스템 설정에서 돌아올 때 권한을 다시 확인합니다. 개발 빌드를 새로 만든 뒤 허용 스위치는 켜져 있는데 계속 적용되지 않으면, **시스템 설정 → 개인정보 보호 및 보안 → 화면 및 시스템 오디오 녹음**에서 기존 Ouento 항목을 제거하고 현재 빌드의 `Ouento.app`을 다시 추가하세요. 빌드 간 권한을 유지하려면 동일한 개발 서명 인증서를 사용해야 합니다([플랫폼 설명](docs/platform-contract.md)).
