...
**성격**에서 캐릭터 이름·나를 부르는 호칭·관계·외형 설명·성격·말투·상황별 대사를 설정합니다. 새 설치의 기본은 **장난스러운 츤데레 여동생**, 기본 호칭은 수정 가능한 **오빠**입니다. 기존 설치의 성격 선택은 유지합니다. 프리셋을 고르고 **선택한 성격 예시 불러오기**를 누르면 편집 가능한 예시가 채워지며, **이 성격으로 함께하기**를 눌러 저장합니다. 외형 설명은 AI의 자기 묘사에 쓰이며 실제 Live2D 모델은 캐릭터 메뉴에서 바꿉니다. [성격 설정과 프롬프트 안내](docs/personality.md).
