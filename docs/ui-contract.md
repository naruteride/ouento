...
- 데스크톱 말풍선은 `src/ui/speech-bubble.js`가 관리한다. 표시 시간은 연속 공백을 한 칸으로 정규화한 Unicode grapheme 수를 기준으로 `max(9초, 2초 + 글자 수 × 0.1초)`다. 음성 재생·일시정지 중에는 유지하고 음성이 끝나면 같은 읽기 시간을 다시 제공한다. 등장은 아래 8px에서 원위치로 200ms fade-in, 퇴장은 아래 8px로 400ms fade-out한다. 동작 줄이기 설정에서는 이동 없이 투명도만 전환한다. 새 대사는 이전 타이머를 무효화하고, 잠금·캐릭터 숨김·앱 종료에서는 즉시 정리한다.
- 말풍선은 82% 불투명도의 크림색 배경, 최대 너비 300px, 13px 글꼴, 세로 10px/가로 14px 안쪽 여백을 사용한다. 네이티브에서는 `speech-bubble` 투명 창으로 표시하고 포커스를 가져오지 않는다. 캐릭터 머리 위를 기본 위치로 삼되 캐릭터가 가장자리·화면 밖에 있으면 말풍선만 현재 또는 가장 가까운 모니터의 작업 영역 안에 보정한다. 그림자·8px 퇴장 이동 여백도 창 크기에 포함한다. 캐릭터의 자유로운 화면 밖 드래그는 유지한다. `src/ui/bubble-surface.js`는 위치 측정과 표시만 담당하며 원래 캐릭터 컨트롤러의 유지 시간·음성 상태·취소 세대를 그대로 따른다.
